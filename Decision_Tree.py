...
# 2. Đường dẫn dataset
data_dir = "/kaggle/input/chest-xray-pneumonia/chest_xray/chest_xray"   # chỉnh lại theo thư mục của bạn
...
plt.show()
# ======================================================
# PHẦN 2: HUẤN LUYỆN & ĐÁNH GIÁ VỚI DECISION TREE
# ======================================================
...
dt = DecisionTreeClassifier(random_state=42, max_depth=20)  # có thể thay đổi max_depth
...
# Vẽ một cây quyết định nhỏ (chỉ hiển thị nếu max_depth nhỏ)
plt.figure(figsize=(12,6))
plot_tree(dt, max_depth=2, feature_names=[f"PC{i+1}" for i in range(100)],
          class_names=categories, filled=True)
plt.show()
# ======================================================
# PHẦN 3: DỰ ĐOÁN ẢNH MỚI VỚI DECISION TREE
# ======================================================

def predict_image(img_path, model, scaler, pca, categories, img_size=64):
...
# Ví dụ dự đoán với một ảnh bất kỳ trong tập test
test_img_path = os.path.join(data_dir, "test", "NORMAL", random.choice(os.listdir(os.path.join(data_dir, "test", "NORMAL"))))
print("Ảnh test:", test_img_path)
predict_image(test_img_path, dt, scaler, pca, categories)
